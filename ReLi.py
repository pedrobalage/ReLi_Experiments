...
class OldCorpusReader(dict):
    ...
    def __init__(self, path='ReLi/corpus/'):
    ...
    def build_corpus(self,path,output_format = 'word'):
    ...
    def __is_predicate__(self,word_structure):
        try:
            word,pos,obj,opinion,pol,_help = word_structure
        except:
            print('Wrong word_structure format')
            return False
    ...
    def __is_aspect__(self,word_structure):
        try:
            word,pos,obj,opinion,pol,_help = word_structure
        except:
            print('Wrong word_structure format')
    ...
    # return sentences in conll format
    #  CONLL format: (S
    def conll_format_aspects(self, sentences):
        ...
        for sentence in sentences:
            ...
            for index, word_structure in enumerate(sentence):

                word, pos = word_structure[0:2]
                tag_word = word.replace('/','-')+'/'+pos
                tag_word = tag_word.replace('[','.')
                tag_word = tag_word.replace(']','.')

                if self.__is_aspect__(word_structure):
                    if is_aspect:
                        conll_sentence += ' ' + tag_word
                    else:
                        is_aspect = True
                        conll_sentence += new_line + '[ ' + tag_word
                    if index < len(sentence)-1:
                        if not self.__is_aspect__(sentence[index+1]):
                            conll_sentence += ' ]'
                    else:
                        conll_sentence += ' ]'
                else:
...
#                # Look for predicates
#                if self.__is_predicate__(word_structure):
#                    if is_predicate:
#                        conll_sentence += ' ' + tag_word
#                    else:
#                        is_predicate = True
#                        conll_sentence += new_line + '(PREDICATE ' + tag_word
#                    if index < len(sentence)-1:
#                        if not self.__is_predicate__(sentence[index+1]):
#                            conll_sentence += ' )'
#                    else:
#                        conll_sentence += ' )'
#                else:
#                        is_predicate = False
                ...
                if  not is_predicate and not is_aspect:
                    conll_sentence += new_line + tag_word
    ...
    # return sentences in conll format
    #  CONLL format: (S
    def conll_format(self, sentences):
            ...
            for index, word_structure in enumerate(sentence):
                ...
                if self.__is_aspect__(word_structure):
                    if is_aspect:
                        ...
                        is_aspect = True
                        conll_sentence += new_line + '(ASPECT ' + tag_word
                    if index < len(sentence)-1:
                        if not self.__is_aspect__(sentence[index+1]):
                            conll_sentence += ' )'
                    else:
...
#                        is_predicate = False
    ...
    def convertHtml(self, path='ReLi/',output_path='ReLiWeb/'):
